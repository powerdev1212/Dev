...
from experts.models import ExpertInstitution
from loaders.models import LoaderEngine
from loaders.handlers import PublicationsUploadHandler
from taxonomy.models import Term


class DeseasesCategoryView(ListView):

    template_name = "deseases/category.html"
    ...
    def get_nci_guidelines(self):
        ...
        def _get_nci_guidelines(node, stack, excluded_ids):
            excluded_ids = [i.id for i in stack]
            for treatment in node.treatments.all():
                for guideline in treatment.guidelines.filter(issuer__inner_name__iexact="nci", active=True):
    ...
    def get_nccn_guidelines(self):
        ...
        for treatment in self.ditem.treatments.all():
            ...
            .filter(issuer__inner_name__iexact="nccn", active=True):
    ...
    def get_context_data(self, **kwargs):
                vdata = super(DeseasesCategoryView, self).get_context_data(**kwargs)
